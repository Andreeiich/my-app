...
const Cart = (props) => {
    ...
    const onAddOverlay = async (obj)=>{ 
        ...
        try{
            ...
            if(findOverlay){
                axios.delete(`https://637f91cc2f8f56e28e904eb7.mockapi.io/cart/${findOverlay.id}`)
      props.setOverlayItems((over) => over.filter(item => item.myId !== obj.myId));
           
        }else{
            const{data} = await axios.post('https://637f91cc2f8f56e28e904eb7.mockapi.io/cart', obj)
    ...
    const onAddFav = async (obj)=>{
        ...
        try{
            ...
            if(findFavorite){
                axios.delete(`https://637f91cc2f8f56e28e904eb7.mockapi.io/favorites/${findFavorite.id}`)
                props.setFavorites((over) => over.filter(item => item.myId !== obj.myId));
            }else{
                const {data} = await axios.post('https://637f91cc2f8f56e28e904eb7.mockapi.io/favorites', obj)
